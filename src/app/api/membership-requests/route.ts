...
export async function POST(req: Request) {
  try {
    ...
    const pending = await MembershipRequest.findOne({ email, status: "pending" }).lean();
    if (pending) {
      return NextResponse.json({ ok: false, error: "A pending request already exists for this email." }, { status: 409 });
    }
