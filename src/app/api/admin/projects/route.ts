...
export async function POST(req: Request) {
  ...
  if (exists) {
    return NextResponse.json({ ok: false, error: "A project with a similar title already exists." }, { status: 409 });
  }
  ...
  return NextResponse.json({ ok: true, id: String(item._id), slug: item.slug }, { status: 201 });
}
