...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (parsed.data.month) {
    const exists = await Report.findOne({ month: parsed.data.month, _id: { $ne: params.id } }).lean();
    if (exists) {
      return NextResponse.json({ ok: false, error: "Another report already uses this month." }, { status: 409 });
    }
  }
  ...
  if (parsed.data.openingBalance !== undefined) doc.openingBalance = parsed.data.openingBalance;
  if (parsed.data.income !== undefined) doc.income = parsed.data.income as any;
  if (parsed.data.expense !== undefined) doc.expense = parsed.data.expense as any;
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions);
  if ((session?.user as any)?.role !== "Admin") {
    return NextResponse.json({ ok: false, error: "Forbidden" }, { s
